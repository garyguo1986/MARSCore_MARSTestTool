codeunit 1059707 "MARS Test Runner Prog. Dialog"
{
    ...
    [EventSubscriber(ObjectType::Codeunit, Codeunit::"MARS Test Runner - Mgt", 'OnRunTestSuite', '', false, false)]
    local procedure OpenWindow(var TestMethodLine: Record "MARS Test Method Line")
    var
        CopyTestMethodLine: Record "MARS Test Method Line";
    begin
    ...
    [EventSubscriber(ObjectType::Codeunit, Codeunit::"MARS Test Runner - Mgt", 'OnAfterRunTestSuite', '', false, false)]
    local procedure CloseWindow(var TestMethodLine: Record "MARS Test Method Line")
    begin
        if not GuiAllowed() then
            exit;
    ...
    [EventSubscriber(ObjectType::Codeunit, Codeunit::"MARS Test Runner - Mgt", 'OnAfterTestMethodRun', '', false, false)]
    local procedure UpDateWindow(var CurrentTestMethodLine: Record "MARS Test Method Line"; CodeunitID: Integer; CodeunitName: Text[30]; FunctionName: Text[128]; FunctionTestPermissions: TestPermissions; IsSuccess: Boolean)
    begin
        if not GuiAllowed() then
            exit;
